...
// Nasty test case with lots of long thin triangles.  The glb format is limited
// to float precision, so by definition these are floating point precision
// vertex positions.  See the SlowOffsetDouble test for a higher precision
// version of this same test.
TEST(BooleanComplex, SlowOffsetFloat) {
  std::string file = __FILE__;
  std::string dir = file.substr(0, file.rfind('/'));
  MeshGL seeds = ImportMesh(dir + "/models/" + "Generic_Twin_7316.1.t0.glb");
  EXPECT_TRUE(seeds.NumTri() > 10);
  EXPECT_TRUE(seeds.NumVert() > 10);
  // Unique edges
  std::vector<std::pair<int, int>> edges;
  ...
  // Extract a Mesh
  manifold::MeshGL omesh = c.GetMeshGL();
}

// Same as SlowOffsetFloat, except OBJ file has full double precision
TEST(BooleanComplex, SlowOffsetDouble) {
  std::string file = __FILE__;
  std::string dir = file.substr(0, file.rfind('/'));
  MeshGL seeds = ImportMesh(dir + "/models/" + "Generic_Twin_7316.1.t0.obj");
  EXPECT_TRUE(seeds.NumTri() > 10);
  EXPECT_TRUE(seeds.NumVert() > 10);
  // Unique edges
  std::vector<std::pair<int, int>> edges;
