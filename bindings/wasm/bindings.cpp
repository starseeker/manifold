...
EMSCRIPTEN_BINDINGS(whatever) {
  value_object<glm::dvec2>("vec2").field("x", &glm::dvec2::x).field("y", &glm::dvec2::y);

  value_object<glm::vec<3, int>>("ivec3")
      .field("0", &glm::vec<3, int>::x)
      .field("1", &glm::vec<3, int>::y)
      .field("2", &glm::vec<3, int>::z);
  ...
  register_vector<glm::vec<3, int>>("Vector_ivec3");
  ...
  function(
      ...
      select_overload<CrossSection(std::vector<glm::dvec2>)>(&CrossSection::Hull));
  ...
  function(
      "_manifoldHullPoints",
      select_overload<Manifold(const std::vector<glm::dvec3>&)>(&Manifold::Hull));
