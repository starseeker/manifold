...
std::tuple<Vec<int>, Vec<int>> SizeOutput(
  ...
  if (invertQ) {
    gather(next, end,
           TransformIterator(inQ.faceNormal_.begin(), Negate<glm::dvec3>()),
...
void CreateProperties(Manifold::Impl &outR, const Manifold::Impl &inP,
  ...
  using Entry = std::pair<glm::vec<3, int>, int>;
  ...
  for (int tri = 0; tri < numTri; ++tri) {
    ...
    const glm::vec<3, int> &triProp = oldNumProp == 0 ? glm::vec<3, int>(-1)
    ...
    for (const int i : {0, 1, 2}) {
      ...
      glm::vec<4, int> key(PQ, idMissProp, -1, -1);
      if (oldNumProp > 0) {
        int edge = -2;
        for (const int j : {0, 1, 2}) {
          if (uvw[j] == 1) {
      ...
      if (key.y == idMissProp && key.z >= 0) {
        ...
        for (const auto &b : bin) {
          if (b.first == glm::vec<3, int>(key.x, key.z, key.w)) {
            bFound = true;
        ...
        if (bFound) continue;
        bin.push_back(std::make_pair(glm::vec<3, int>(key.x, key.z, key.w), idx));
      }
