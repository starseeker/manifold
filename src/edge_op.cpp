...
glm::vec<3, int> TriOf(int edge) {
  glm::vec<3, int> triEdge;
  triEdge[0] = edge;
...
struct SwappableEdge {
  ...
  bool operator()(int edge) const {
    ...
    int tri = edge / 3;
    glm::vec<3, int> triEdge = TriOf(edge);
...
void Manifold::Impl::CollapseTri(const glm::vec<3, int>& triEdge) {
...
void Manifold::Impl::RemoveIfFolded(int edge) {
  const glm::vec<3, int> tri0edge = TriOf(edge);
  const glm::vec<3, int> tri1edge = TriOf(halfedge_[edge].pairedHalfedge);
...
void Manifold::Impl::CollapseEdge(const int edge, std::vector<int>& edges) {
  ...
  Vec<glm::vec<3, int>>& triProp = meshRelation_.triProperties;
  ...
  const glm::vec<3, int> tri0edge = TriOf(edge);
  const glm::vec<3, int> tri1edge = TriOf(toRemove.pairedHalfedge);
  ...
  if (!shortEdge) {
    ...
    while (current != tri0edge[2]) {
      ...
      const TriRef ref = triRef[tri];
      const glm::dmat3x2 projection = GetAxisAlignedProjection(faceNormal_[tri]);
...
void Manifold::Impl::RecursiveEdgeSwap(const int edge, int& tag,
  ...
  const glm::vec<3, int> tri0edge = TriOf(edge);
  const glm::vec<3, int> tri1edge = TriOf(pair);
  const glm::vec<3, int> perm0 = TriOf(edge % 3);
  const glm::vec<3, int> perm1 = TriOf(pair % 3);
