...
inline double cross(glm::dvec2 p, glm::dvec2 q) { return p.x * q.y - p.y * q.x; }

// return true if p intersects with q
// note that we don't care about collinear, intersection in the ends etc.
// https://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
inline bool intersect(glm::dvec2 p0, glm::dvec2 p1, glm::dvec2 q0, glm::dvec2 q1, double precision) {
  glm::dvec2 r = p1 - p0;
  glm::dvec2 s = q1 - q0;
...
void DumpTriangulation(const Polygons &polys, double precision) {
  ...
  std::vector<glm::vec<3, int>> result = Triangulate(polys);
  PolygonParams() = oldParams;
  for (const glm::vec<3, int> &tri : result) {
...
void simplify(Polygons &polys, double precision = -1) {
  ...
  while (removedSomething) {
    ...
    for (size_t i = 0; i < polys.size(); i++) {
      ...
      try {
        std::vector<glm::vec<3, int>> result = Triangulate(polys, precision);
