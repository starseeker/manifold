...
class HullImpl {
 public:
  ...
  virtual void hull(const manifold::Manifold &input,
                    const std::vector<glm::dvec3> &pts) = 0;
  ...
  bool isMeshConvex() {
    ...
    for (const auto &tri : mesh.triVerts) {
      ...
      glm::dvec3 v0 = vertPos[tri[0]];
      glm::dvec3 v1 = vertPos[tri[1]];
      glm::dvec3 v2 = vertPos[tri[2]];
...
class HullImplOriginal : public HullImpl {
 public:
  void hull(const manifold::Manifold &input,
            const std::vector<glm::dvec3> &pts) override {
    inputManifold = input;
